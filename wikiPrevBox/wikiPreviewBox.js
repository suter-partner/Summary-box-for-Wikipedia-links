 ...
 * su-pa.net (c) version 11/22/2021, nick, MIT-License
...
{//simple(st) "let namespace" => use let or const inside this brakets    
    ...
    //document.getElementsByTagName('head')[0].insertAdjacentHTML('beforeend','<link rel="stylesheet" type="text/css" href="/wikiPreviewBox.min.css">');
    ...
    if (typeof (document.currentScript.dataset.wikipreview) != "undefined") {
        ...
        //how to use: <script data-wikipreview="width=250,noimages" src="... ></script>       
    }

    //max number of characters in the wiki preview box
    let wikiBoxMaxLenght = 280; //includes some HTML tags (~15... signs <p><b>..., + on disambiguation terms + <ul>, <li>..., ...)
    ...
    //#region add events to wiki-links, some positioning of a preview box
    ...
    //"cleaning": if there are links with an added "#nopreview" => clean up the links & remove "#nopreview"
    ...
    for (let i = 0; i < allWikiLinks.length; i++) {
        ...
        allWikiLinks[i].addEventListener("mouseover", (e) => {
            ...
            //show wiki preview box below wiki-tag
            ...
            if (winInnerWidth < wikiBoxWidth) { wikiBoxLeft = winInnerWidth - wikiBoxWidth + (wikiBoxWidth - winInnerWidth); }
                ...
                if ((wikiBoxLeft + wikiBoxWidth) > winInnerWidth) { wikiBoxLeft = winInnerWidth - wikiBoxWidth - 10; } //do not pos out right side
            }
            wikiPreviewBox.style.left = wikiBoxLeft + "px"; //maybe move this (all) to the function in the bottom
            //#endregion
                ...
                .then(response => {                    
                    if (response.status >= 200 && response.status <= 299) { //ok, no network error (but neverthless could be a 404 error)
                        response.json().then(wikiSummData => {
                            ...
                            createWikiBox(arrArticleSummary);
                        })
                    }
    ...
    function createWikiBox(arrArticleSum) {
        let wikiBoxContent = "";
        ...
        if (arrArticleSum[1].length <= wikiBoxMaxLenght) {
            ...
            wikiBoxContent += "&nbsp;..." + lastClosingTag; // +lastClosingTag: '</li>' is not needed because browsers close <li> automatically (faster) in this case 
        }
        wikiPreviewBox.innerHTML = wikiBoxContent;

        let wikiBoxfooter = "<span class='wikiBoxfooter'><a href='" + arrArticleSum[2] + "' target='_blank' rel='noopener'>" + arrArticleSum[3] +
            "</a> (wikipedia.org)</span><a href='https://su-pa.net/wikiPrevBox/' target='_blank' rel='noopener'><img class='wikiBoxLogo' src='https://su-pa.net/wikiPrevBox/wikiLogo.svg' title='about Wikipedia preview box - click for info'>";
